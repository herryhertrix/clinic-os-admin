...
test.describe('AnamnesisFormDetail', () => {
  ...
  test('should render form details', async ({ page }) => {
    ...
    await expect(page.locator("text=Questions about your family's medical history")).toBeVisible();
    await expect(page.locator('text=Section 1')).toBeVisible();
    await expect(page.locator('text=Multiple choice: Does your family have a history of heart disease?')).toBeVisible();
  });

  test('should navigate back when "Back" button is clicked', async ({ page }) => {
    await page.click('text=Back');
    const formTitle = await page.textContent('tbody tr:first-child td:first-child');
