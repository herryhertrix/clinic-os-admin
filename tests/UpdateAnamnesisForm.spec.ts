...
test.describe('UpdateAnamnesisForm', () => {
  ...
  test('should allow the user to update an anamnesis form with sections and questions', async ({ page }) => {
    ...
    // Before drag
    const firstSourceSectionText = await sourceSection.evaluate((node: any) => node.value);
    const secondSourceSectionText = await targetSection.evaluate((node: any) => node.value);
    ...
    // After drag
    const firstSectionText = await page.locator('input[placeholder="Section Title"]').nth(0).evaluate((node: any) => node.value);
    const secondSectionText = await page.locator('input[placeholder="Section Title"]').nth(1).evaluate((node: any) => node.value);
    ...
    const sourceQuestion = await page.locator('input[placeholder="Question Text"]').nth(1);
    const targetQuestion = await page.locator('input[placeholder="Question Text"]').nth(0);

    // Before drag
    const firstSourceQuestionText = await sourceQuestion.evaluate((node: any) => node.value);
    const secondSourceQuestionText = await targetQuestion.evaluate((node: any) => node.value);
    ...
    // After drag
    const firstQuestionText = await page.locator('input[placeholder="Question Text"]').nth(0).evaluate((node: any) => node.value);
    const secondQuestionText = await page.locator('input[placeholder="Question Text"]').nth(1).evaluate((node: any) => node.value);
