...
const CreateAnamnesisForm = ({ forms, setForms, handlePreviousPage, handleNextPage }: any) => {
    ...
    setSections((items: any) => {
      return items.map((section: any) => {
        if (section.id === sectionId) {
          const oldIndex = section.questions.findIndex((question: any) => question.id === active.id);
          const newIndex = section.questions.findIndex((question: any) => question.id === over.id);
          return {
            ...section,
  ...
  return (
    <div className="mx-auto p-4">
      <h1 className="text-3xl font-bold text-center my-4">Create Anamnesis Form</h1>
          ...
          {sections.map((section: any, sectionIndex: number) => (
            <Draggable key={section.id} id={section.id}>
              <div className="border p-2 mb-2">
                ...
                />
                <DndContext sensors={sensors} onDragEnd={(event: any) => handleDragQuestionEnd(section.id, event)}>
